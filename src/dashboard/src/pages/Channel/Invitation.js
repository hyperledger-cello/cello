...
const Invitation = ({
  ...
  const fetchInvitations = useCallback(() => {
    refreshList(channelId ? { channelId } : {});
  ...
  const runAction = useCallback(
    (actionType, record, successMsgId, successDefault, failMsgId, failDefault) => {
      const actionChannelId = channelId || (record.channel && record.channel.id);
      setOperatingId(record.id);
      setOperationType(actionType);
      dispatch({
        type: `invitation/${actionType}Invitation`,
        payload: { channelId: actionChannelId, invitationId: record.id },
        callback: response => {
          setOperatingId(null);
        ...
        <div className={styles.tableList}>
          <div className={styles.tableListOperator}>
            ...
            <Select
              ...
              onChange={v => {
                setChannelId(v);
                history.push(`/channel/invitation${v ? `?channel=${v}` : ''}`);
              }}
              showSearch
            ...
            >
              <Option value={null}>
                {intl.formatMessage({
                  id: 'app.channel.invitation.selectAllChannels',
