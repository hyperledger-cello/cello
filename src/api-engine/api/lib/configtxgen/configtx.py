...
class ConfigTX:
    ...
    def __init__(
        ...
        orderer=None,
        raft_option=None,
        template_path="/opt/config/configtx.yaml",
    ):
        """init ConfigTX
        param:
    ...
    def create(
        ...
        for orderer in orderers:
            ...
            OrdererOrg = dict(
                Name="Orderer",
                ID=OrdererMSP,
                MSPDir="{}/{}/crypto-config/ordererOrganizations/{}/msp".format(
                    self.filepath,
                    orderer["name"],
                    orderer["name"].split(".", 1)[1],
                ),
                Policies=dict(
                    Readers=dict(
                        Type="Signature",
                        Rule="OR('{}.member')".format(OrdererMSP),
                    ),
                    Writers=dict(
                        Type="Signature",
                        Rule="OR('{}.member')".format(OrdererMSP),
                    ...
                    Admins=dict(
                        Type="Signature",
                        Rule="OR('{}.admin')".format(OrdererMSP),
                    ),
                ),
            )
            for host in orderer["hosts"]:
                OrdererAddress.append(
                    "{}.{}:{}".format(
                        host["name"], orderer["name"].split(".", 1)[1], 7050
                    )
                )
                Consenters.append(
                    dict(
                        Host="{}.{}".format(
                            host["name"], orderer["name"].split(".", 1)[1]
                        ),
                        Port=7050,
        ...
        for peer in peers:
            ...
            PeerOrganizations.append(
                dict(
                    Name=peer["name"].split(".", 1)[0].capitalize(),
                    ID=PeerMSP,
                    MSPDir="{}/{}/crypto-config/peerOrganizations/{}/msp".format(
                        self.filepath, peer["name"], peer["name"]
                    ),
                    Policies=dict(
                        Readers=dict(
                            Type="Signature",
                            Rule="OR('{}.admin', '{}.peer', '{}.client')".format(
                                PeerMSP, PeerMSP, PeerMSP
                            ),
                        ),
                        Writers=dict(
                            Type="Signature",
                            Rule="OR('{}.admin', '{}.client')".format(
                                PeerMSP, PeerMSP
                            ),
                        ),
                        Admins=dict(
                            Type="Signature",
                            Rule="OR('{}.admin')".format(PeerMSP),
                        ),
                        Endorsement=dict(
                            Type="Signature",
                            Rule="OR('{}.peer')".format(PeerMSP),
                        ),
                    ),
                )
            )
        ...
        Profiles[name]["Application"]["Capabilities"] = Capabilities[
            "Application"
        ]
        ...
        with open(
            "{}/{}/configtx.yaml".format(self.filepath, self.network),
            "w",
            encoding="utf-8",
        ) as f:
    ...
    def createChannel(self, name, organizations):
        ...
        try:
            with open(
                "{}/{}/{}".format(
                    self.filepath, self.network, "configtx.yaml"
                ),
                "r+",
                encoding="utf-8",
            ) as f:
...
if __name__ == "__main__":
    ...
    ConfigTX("test3").create(
        consensus="etcdraft", orderers=orderers, peers=peers
    )
    # tx = ConfigTX("test3")
    # print(tx.template)
